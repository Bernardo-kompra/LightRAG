...
RUN --mount=type=cache,id=uv-cache-builder-base,target=/root/.local/share/uv \
...
RUN --mount=type=cache,id=uv-cache-builder-project,target=/root/.local/share/uv \
...
RUN --mount=type=cache,id=uv-cache-runtime-sync,target=/root/.local/share/uv \
    uv sync --frozen --no-dev --extra api --extra offline --no-editable \
    && /app/.venv/bin/python -m ensurepip --upgrade
